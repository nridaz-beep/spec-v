...
function report() { return { errors: [], stats: { unexpected: 0, skipped: 0, flaky: 0 }, suites: [{ specs: ['81問','3回失敗','test=1','LATEST','claim','used_at','空本文','通知回復'].map(title => ({ title, ok: true, tests: [{ expectedStatus: 'passed', status: 'expected', results: [{ status: 'passed' }] }] })) }] }; }
test('Only a complete passing report can produce promotion evidence', () => assert.equal(validateReport(report()), 8));
